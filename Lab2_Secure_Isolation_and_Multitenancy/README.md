...
Result:
The ccse-lab2 Kubernetes cluster was created successfully with default CNI disabled
...
Result:
Namespace tenant-a and namespace tenant-b were created
...
Deployment web was created in tenant-a and tenant-b

Service web was exposed on port 80 in both namespaces

Displays the running web pod and active service inside tenant-a
...
Result:
Service IP address for tenant-b was returned
...
Result:
HTTP 200 was returned, proving cross-tenant communication is allowed by default
...
Result:
ResourceQuota tenant-a-quota was created in tenant-a
...
Result:
NetworkPolicy default-deny-ingress was created in tenant-b
...
Result:
Secret data was created in both tenant-a and tenant-b
...
Result:
ServiceAccount app-a was created in tenant-a

Role reader and RoleBinding rb were created in tenant-a
...
Result:
Sensitive text was found in raw memory blocks after standard file deletion.
...
Result:
wiped (Data was completely overwritten with zeros before deletion)
...
Shows active network policies across all namespaces (tenant-b default-deny ingress)

3. Verify Resource Quota:
...
Displays enforced limits for CPU, memory, and pods for tenant-a
...
Conclusion
- 
This lab successfully demonstrated the three main dimensions of cloud security which are compute, network, and storage isolation. Default configurations in Kubernetes leave networks open and pose a security risk in multi-tenant systems. Security rules like Network Policies and RBAC successfully blocked unauthorized cross-tenant network and secret access. Overwriting file bytes proved that proper wiping is required to completely erase sensitive residual data.  
